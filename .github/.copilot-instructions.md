...
FREERANGE_LOG_LEVEL=debug         # Enable debug logging (Hub/Worker)
DEBUG=true                        # Alternative toggle for worker debug mode
...
#### System Endpoints
```
GET  /v1/health             # Service health check with database status
GET  /v1/sse                # SSE connection (requires ?user_id=internal_uuid)
...
**CRITICAL**: There is a distinction between `external_user_id` and `internal_user_id` (UUID).
- `external_user_id`: Provided by the client during user registration.
- `internal_user_id`: Generated by the system on creation. **REQUIRED** for:
  - SSE Connection: `/v1/sse?user_id={internal_uuid}`
  - Notification Payload: `{"user_id": "{internal_uuid}", ...}`
- **Lookup**: Use the GetByExternalID endpoint to find the internal UUID if only the external ID is known.
